...
def build_culture_inventory(
    ...
    try:
        ...
        response = {"inventory": inventory, "fallback": "LLM unavailable; used search snippets directly."}
...
def _fallback_inventory(search_results: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for result in search_results.get("results", []):
        ...
        for snippet in result.get("snippets", []):
            ...
            # Skip if it's clearly a generic blog/list page, not a specific landmark
            skip_patterns = ["攻略", "推荐", "分享", "打卡", "游记", "行程", "Trip.com",
                             "Klook", "Instagram", "Facebook", "博客", "旅游", "自由行"]
            if any(p in raw_title for p in skip_patterns):
                continue
